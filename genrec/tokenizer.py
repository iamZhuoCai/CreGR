...
class SemIDTokenizer(AbstractTokenizer):
    ...
    def load_sent_emb(self, dataset: AbstractDataset):
        ...
        Supports both sentence-transformer encoding and pre-computed embeddings.
        ...
        """
        # Check if dataset provides pre-computed embeddings (e.g., Disco)
        if hasattr(dataset, 'get_item_embeddings'):
            self.log('[TOKENIZER] Using pre-computed item embeddings from dataset')
            sent_embs = dataset.get_item_embeddings()

            # PCA
            if self.config['sent_emb_pca'] > 0:
                self.log(f'[TOKENIZER] Applying PCA to embeddings...')
