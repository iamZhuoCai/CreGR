...
def get_command_line_args_str():
    ...
    if len(args_str) > 100:
        args_str = hashlib.md5(args_str.encode()).hexdigest()[:12]
    return args_str
...
def _coerce_cli_collection(raw: str):
    """Best-effort parse of list/dict-like CLI values when eval() fails.

    Bash often strips quotes so ``--metrics=[ndcg,recall,cr]`` survives as a
    string; iterating that string in the evaluator yields ``'['`` and crashes.
    """
...
def parse_command_line_args(unparsed: list[str]) -> dict:
    ...
    for text_arg in unparsed:
        ...
        if isinstance(value, str) and key != 'run_time':
            coerced = _coerce_cli_collection(value)
...
def init_device():
    ...
    if torch.cuda.is_available():
        ...
        return torch.device('mps'), use_ddp
    else:
